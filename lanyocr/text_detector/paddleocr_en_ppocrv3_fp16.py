...
class PaddleOcrEnPPOCRV3FP16(LanyOcrDetector):
    def __init__(self, use_gpu: bool = True) -> None:
        super().__init__(use_gpu)

        print("Detector: PaddleOcrEnPPOCRV3-FP16")

        model_path = download_model("lanyocr-en-ppocrv3-det_FP16.onnx")
    ...
    def infer(self, image) -> List[LanyOcrRRect]:
        ...
        preds = self.session.run(None, input_dict)[0].astype(np.float32)
    ...
    def preprocess(self, img: np.ndarray):
        ...
        return img.astype(np.float16), np.array([src_h, src_w, ratio_h, ratio_w])
